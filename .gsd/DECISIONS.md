...
- "Checkpoint integration activates only when checkpointPath != '' AND len(units) > 1 — avoids checkpoint overhead for single-account/subscription/project scans"
- "Swappable func vars (discoverAccountsFunc, getAccountIDFunc, scanOneAccountFunc) added to AWS scanner for checkpoint testability — matches Azure's existing scanSubscriptionFunc pattern"
- "scanOneProjectFunc swappable var added to GCP scanner — same pattern as Azure/AWS for testing fan-out without real API calls"
- "Checkpoint atomicSave uses write-to-tmp + os.Rename — prevents partial reads on crash; tmp file in same directory guarantees same-filesystem rename"
- "Checkpoint Version=1 with forward-compatibility guard — Load rejects unknown versions with descriptive error instead of silently loading incompatible data"
- "Checkpoint Load returns (nil, nil) for non-existent file — callers treat as fresh scan start without special error handling"
- "CheckpointPath uses json tag checkpointPath,omitempty on ScanProviderSpec — empty string omitted from API JSON for backward compatibility with old clients"
- "Per-type DNS items use lowercase underscore naming (dns_record_a, dns_record_cname) matching NIOS families.go convention — not dns_record_A"
- "Cloud scanners emit per-type DNS FindingRows (dns_record_a, dns_record_ds, etc.) for both supported and unsupported types — no supported/unsupported grouping like bluecat/efficientip"
