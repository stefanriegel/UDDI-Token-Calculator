...
- Confirm reference data path exists at `/Users/mustermann/Documents/coding/Infoblox-Universal-DDI-cloud-usage/`. If absent, NIOS / EfficientIP / BlueCat upload sub-tests will be skipped (per D-04) — note this in the report header.
...
const REFERENCE = '/Users/mustermann/Documents/coding/Infoblox-Universal-DDI-cloud-usage';
const VIEWPORTS = [
...
const REFERENCE_AVAILABLE = fs.existsSync(REFERENCE);
...
(async () => {
  ...
  console.log(`Pre-flight ✓ — backend mode: ${backendMode}, reference: ${REFERENCE_AVAILABLE ? 'available' : 'MISSING (upload sub-tests will skip)'}`);
...
Expected: prints `Pre-flight ✓ — backend mode: demo, reference: available` (or `MISSING`), opens chromium briefly, exits 0, writes the empty `/tmp/uddi-uxaudit/<stamp>/` dirs.
...
const SCAN_PROVIDERS = [
  ...
  { name: 'NIOS', label: 'Infoblox NIOS Grid', upload: true, fixture: `${REFERENCE}/path-to-nios-backup.tar.gz` },
  { name: 'BlueCat', label: 'BlueCat Address Manager', upload: false }, // BlueCat uses creds, not file
  { name: 'EfficientIP', label: 'EfficientIP SOLIDserver', upload: true, fixture: `${REFERENCE}/path-to-efficientip-backup.gz` },
];
...
async function scanWalk(browser, prov, vp) {
  ...
  if (prov.upload) {
    if (!REFERENCE_AVAILABLE) {
      mark('Scan', prov.name, '2 Credentials', vp.name, 'skipped', 'reference fixture missing');
      await ctx.close();
      return;
    }
...
**reference fixtures:** ${REFERENCE_AVAILABLE ? 'available' : 'MISSING — upload paths skipped'}
...
ls /Users/mustermann/Documents/coding/Infoblox-Universal-DDI-cloud-usage 2>/dev/null && echo reference-ok || echo reference-missing
```
