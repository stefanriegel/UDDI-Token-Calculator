...
| D-04 | Upload-path fixtures: reference local data symlinked from `/Users/mustermann/Documents/coding/Infoblox-Universal-DDI-cloud-usage/...` per project memory. Skip upload paths gracefully if path missing. No fixtures committed. |
...
3. reference data path exists. If missing, mark NIOS / BlueCat / EfficientIP upload sub-tests as `skipped` in the coverage matrix and continue with the rest.
...
**Upload-path providers** (NIOS / EfficientIP / BlueCat): identical to above, but at step 2 use `setInputFiles()` to upload the reference fixture, wait for upload-success state. If reference path missing, sub-test marked `skipped` (not failed).
...
6. reference-fixture-dependent tests are skipped (not failed) when the local path is missing.
