...
export function Wizard() {
  ...
  const hybridScenario = useMemo(() => {
    ...
    return { mgmt: Math.ceil(hybridMgmt * (1 + growthBufferPct)) + stayingMgmt, srv: hybridSrv, selectionCount };
  }, [effectiveFindings, effectiveNiosMetrics, effectiveADMetrics, niosMigrationMap, adMigrationMap, selectedProviders, growthBufferPct]);
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            {backend.updateStatus === 'done' ? (
              ...
              <a
                href={backend.updateInfo.releaseURL || '#'}
                target="_blank"
                ...
                title="Run: docker compose pull && docker compose up -d"
              >
                <ArrowUpCircle className="w-3 h-3" />
                <span className="hidden sm:inline">{backend.updateInfo.latestVersion} available</span>
              </a>
                ...
                <div className="sticky top-0 z-10 bg-white border-b border-[var(--border)] rounded-xl mb-6 px-4 py-2.5 flex items-center gap-2 flex-wrap">
                  {[
                    ...(selectedProviders.includes('nios') ? [
                      ...
                      { id: 'section-member-details', label: 'Member Details' },
                    ] : []),
                    ...(selectedProviders.includes('microsoft') && effectiveADMetrics.length > 0 ? [
              ...
              {selectedProviders.includes('nios') && (() => {
                ...
                return (
                  <div id="section-migration-planner" className="bg-white rounded-xl border-2 border-[var(--infoblox-blue)]/30 mb-6 overflow-hidden">
                    ...
                    {niosGridFeatures && (() => {
                      const [featuresOpen, setFeaturesOpen] = [gridFeaturesOpen, setGridFeaturesOpen];
                          ...
                          return (
                            ...
                            <div className="flex items-center gap-2.5 px-3 py-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                                  ...
                                  {serviceBadges.map(b => (
                                    <span key={b.label} className={`inline-block px-1.5 py-0 rounded text-[9px] ${b.bg} ${b.text}`} style={{ fontWeight: 600, lineHeight: '16px' }}>
                                      {b.label}
                                    </span>
                                  ))}
                                </div>
                            ...
                            {memberMetrics && (
                              <div className="flex items-center gap-3 px-3 pb-2 pt-0.5 text-[10px] text-[var(--muted-foreground)] flex-wrap">
                                ...
                                <span title="DDI Objects">DDI: {memberMetrics.objectCount.toLocaleString()}</span>
                                <span className="text-gray-300">|</span>
                                <span title="Active IPs (DHCP leases)">Active IPs: {memberMetrics.activeIPCount.toLocaleString()}</span>
                                <span className="text-gray-300">|</span>
                                <span title="Queries per second">QPS: {memberMetrics.qps.toLocaleString()}</span>
                                <span className="text-gray-300">|</span>
                                <span title="Leases per second">LPS: {memberMetrics.lps.toLocaleString()}</span>
                              </div>
                            )}
                            </div>
                          );
                    ...
                    {(() => {
                      ...
                      const fullSrvTokens = migrateableMetrics.reduce(
                    ...
                    {/* Grid Member Details — collapsible section showing per-member metrics */}
                    {effectiveNiosMetrics.length > 0 && (() => {
                      const allMembers = effectiveNiosMetrics;
                      const detailFiltered = memberDetailsSearch
                        ? allMembers.filter(m => m.memberName.toLowerCase().includes(memberDetailsSearch.toLowerCase()) ||
                            m.role.toLowerCase().includes(memberDetailsSearch.toLowerCase()))
                        : allMembers;

                      const detailRoleColors: Record<string, string> = {
                        GM: '#002B49',
                        GMC: '#1a4a6e',
                        DNS: '#0078d4',
                        DHCP: '#00a5e5',
                      ...
                      return (
                        <div id="section-member-details" className="border-t border-[var(--border)]">
                          <button
                            type="button"
                            onClick={() => setMemberDetailsOpen(v => !v)}
                          ...
                          >
                            ...
                            <span className="ml-1 text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-[var(--muted-foreground)]" style={{ fontWeight: 500 }}>
                              {allMembers.length}
                            </span>
                            <span className="ml-auto text-[11px] text-[var(--muted-foreground)]">
                          ...
                          {memberDetailsOpen && (
                            <div className="px-4 pb-4">
                              ...
                              <div className="relative mb-3">
                                ...
                                <input
                                  type="text"
                                  placeholder="Search members by hostname or role..."
                                  value={memberDetailsSearch}
                                  onChange={(e) => setMemberDetailsSearch(e.target.value)}
                                  className="w-full pl-8 pr-3 py-2 text-[12px] rounded-lg border border-[var(--border)] focus:outline-none focus:ring-1 focus:ring-[var(--infoblox-blue)] focus:border-[var(--infoblox-blue)]"
                                />
                              </div>

                              {detailFiltered.length === 0 ? (
                                <div className="text-center py-8 text-[12px] text-[var(--muted-foreground)]">
                                  No members match the search filter.
                                </div>
                              ) : (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {detailFiltered.map((member) => {
                                    const memberFindings = effectiveFindings.filter(f => f.provider === 'nios' && f.source === member.memberName);
                                    const memberTokens = memberFindings.reduce((s, f) => s + f.managementTokens, 0);
                                    const tier = calcServerTokenTier(member.qps, member.lps, serverSizingObjects(member), 'nios-x');
                                    const infraOnly = isInfraOnlyMember(member);

                                    return (
                                      <div
                                        key={member.memberId}
                                        className={`rounded-xl border ${infraOnly ? 'border-gray-200 bg-gray-50/50' : 'border-[var(--border)] bg-white'} p-4 hover:shadow-sm transition-shadow`}
                                      >
                                        {/* Header: hostname + role */}
                                        <div className="flex items-start justify-between gap-2 mb-3">
                                          <div className="min-w-0 flex-1">
                                            <div className="text-[13px] truncate" style={{ fontWeight: 600 }}>{member.memberName}</div>
                                            {infraOnly && (
                                              <div className="text-[10px] text-amber-600 mt-0.5">Infrastructure-only (no DNS/DHCP workload)</div>
                                            )}
                                          </div>
                                          <span
                                            className="inline-block px-2.5 py-0.5 rounded text-[10px] text-white shrink-0"
                                            style={{ fontWeight: 600, backgroundColor: detailRoleColors[member.role] || '#666' }}
                                          >
                                            {member.role}
                                          </span>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-3 gap-x-4 gap-y-2">
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>DDI Objects</div>
                                          ...
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>Active IPs</div>
                                            <div className="text-[15px] tabular-nums" style={{ fontWeight: 600 }}>
                                              {(member.activeIPCount ?? 0) > 0 ? (member.activeIPCount ?? 0).toLocaleString() : <span className="text-gray-300">&mdash;</span>}
                                            </div>
                                          </div>
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>Mgmt Tokens</div>
                                            <div className="text-[15px] tabular-nums text-[var(--infoblox-orange)]" style={{ fontWeight: 600 }}>
                                              {memberTokens > 0 ? memberTokens.toLocaleString() : <span className="text-gray-300">&mdash;</span>}
                                            </div>
                                          </div>
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>
                                              <span className="flex items-center gap-0.5">
                                                <Activity className="w-2.5 h-2.5" /> QPS
                                              </span>
                                            </div>
                                            <div className="text-[15px] tabular-nums" style={{ fontWeight: 600 }}>
                                          ...
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>
                                              <span className="flex items-center gap-0.5">
                                                <Gauge className="w-2.5 h-2.5" /> LPS
                                              </span>
                                            </div>
                                            <div className="text-[15px] tabular-nums" style={{ fontWeight: 600 }}>
                                          ...
                                          <div>
                                            <div className="text-[9px] uppercase tracking-wider text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>Server Tier</div>
                                            <div className="flex items-center gap-1.5 mt-0.5">
                                              <span className={`inline-block px-2 py-0.5 rounded text-[10px] ${
                                                tier.name === 'XL' ? 'bg-red-100 text-red-700' :
                                                tier.name === 'L' ? 'bg-orange-100 text-orange-700' :
                                                tier.name === 'M' ? 'bg-yellow-100 text-yellow-700' :
                                                tier.name === 'S' ? 'bg-green-100 text-green-700' :
                                                tier.name === 'XS' || tier.name === '2XS' ? 'bg-sky-100 text-sky-700' :
                                                'bg-gray-100 text-gray-700'
                                              }`} style={{ fontWeight: 600 }}>
                                                {tier.name}
                                              </span>
                                              <span className="text-[10px] text-emerald-600" style={{ fontWeight: 600 }}>
                                                {tier.serverTokens.toLocaleString()} ST
                                              </span>
                                            </div>
                                          </div>
                                        </div>
                ...
                const totalNiosReplaced = xaasMembers.length; // 1 connection per NIOS member replaced
                      ...
                      <table className="w-full text-[12px]">
                        ...
                        <tbody>
                          ...
                          {niosXMembers.map((member) => {
                            const tier = calcServerTokenTier(member.qps, member.lps, serverSizingObjects(member), 'nios-x');
                            const hasDhcp = member.role.includes('DHCP') || (member.staticHosts + member.dynamicHosts > 0);
                            const hasLicenses = member.licenses && Object.keys(member.licenses).length > 0;
                            const licenseLabels: Record<string, string> = {
                              enterprise: 'Enterprise', dns: 'DNS', dhcp: 'DHCP', rpz: 'RPZ',
                              nios: 'NIOS', cloud: 'Cloud', cloud_api: 'Cloud API', dtc: 'DTC',
                            };
                            return (
                              ...
                              {/* Detail sub-row: Active IPs, DHCP stats, licenses */}
                              {(member.activeIPCount > 0 || member.managedIPCount > 0 || hasDhcp || hasLicenses) && (
                                <tr className="border-b border-gray-100 bg-gray-50/30">
                                  <td colSpan={8} className="px-6 py-1.5">
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-[var(--muted-foreground)]">
                                      {(member.activeIPCount > 0 || member.managedIPCount > 0) && (
                                        <>
                                          <span>UDDI Active IPs: <b className="text-[var(--foreground)]">{member.activeIPCount.toLocaleString()}</b></span>
                                          <span>Managed IPs: <b className="text-[var(--foreground)]">{member.managedIPCount.toLocaleString()}</b></span>
                                        </>
                                      )}
                                      {hasDhcp && (
                                        <>
                                          <span>Static Hosts: <b className="text-[var(--foreground)]">{member.staticHosts.toLocaleString()}</b></span>
                                          <span>Dynamic Hosts: <b className="text-[var(--foreground)]">{member.dynamicHosts.toLocaleString()}</b></span>
                                          <span>DHCP Util: <b className="text-[var(--foreground)]">{(member.dhcpUtilization / 10).toFixed(1)}%</b></span>
                                        </>
                                      )}
                                      {hasLicenses && (
                                        <span className="flex items-center gap-1 flex-wrap">
                                          {Object.entries(member.licenses!).filter(([, v]) => v).map(([key]) => (
                                            <span key={key} className="inline-block px-1.5 py-0.5 rounded bg-muted text-[10px]" style={{ fontWeight: 500 }}>
                                              {licenseLabels[key] || key}
                                            </span>
                                          ))}
                                        </span>
                                      )}
                    ...
                    {/* Grid Member Details — collapsible 2-column card grid */}
                    <div className="border-t border-emerald-200">
                      ...
                      >
                        {showGridMemberDetails ? <ChevronUp className="w-4 h-4 text-emerald-600" /> : <ChevronDown className="w-4 h-4 text-emerald-600" />}
                        <span className="text-[13px] text-emerald-800" style={{ fontWeight: 600 }}>Grid Member Details</span>
                        <span className="text-[11px] text-[var(--muted-foreground)]">
                      ...
                      {showGridMemberDetails && (
                        <div className="px-4 pb-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {displayMembers
                              .filter((m) => !gridMemberDetailSearch || m.memberName.toLowerCase().includes(gridMemberDetailSearch.toLowerCase()))
                              .map((m) => {
                                const tier = calcServerTokenTier(m.qps, m.lps, serverSizingObjects(m), getMemberFF(m.memberName));
                                const activeLicenses = Object.entries(m.licenses || {}).filter(([, v]) => v).map(([k]) => k);
                                return (
                                  <div key={m.memberId} className="rounded-lg border border-[var(--border)] bg-white p-3 hover:shadow-sm transition-shadow">
                                    {/* Header: name + role badge */}
                                    <div className="flex items-center gap-2 mb-2.5">
                                      <div className="truncate text-[12px]" style={{ fontWeight: 600 }}>{m.memberName}</div>
                                      <span
                                        className="inline-block px-2 py-0.5 rounded text-[9px] text-white shrink-0"
                                        style={{ fontWeight: 600, backgroundColor: roleColors[m.role] || '#666' }}
                                      >
                                        {m.role}
                                      </span>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-3 gap-x-3 gap-y-1.5 text-[11px]">
                                      <div>
                                        ...
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.objectCount > 0 ? m.objectCount.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">UDDI Active IPs</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.activeIPCount > 0 ? m.activeIPCount.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">Managed IPs</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.managedIPCount > 0 ? m.managedIPCount.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">Static Hosts</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.staticHosts > 0 ? m.staticHosts.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">Dynamic Hosts</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.dynamicHosts > 0 ? m.dynamicHosts.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">DHCP Util</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.dhcpUtilization > 0 ? `${(m.dhcpUtilization / 10).toFixed(1)}%` : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">QPS</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.qps > 0 ? m.qps.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">LPS</div>
                                        <div className="tabular-nums" style={{ fontWeight: 500 }}>{m.lps > 0 ? m.lps.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                        <div className="text-[var(--muted-foreground)] text-[10px]">Server Tier</div>
                                        <div>
                                          <span className={`inline-block px-1.5 py-0 rounded text-[10px] ${tierColorClass(tier.name)}`} style={{ fontWeight: 600 }}>{tier.name}</span>
                                          <span className="ml-1 text-emerald-600 text-[10px]" style={{ fontWeight: 600 }}>{tier.serverTokens.toLocaleString()} tok</span>
                                        </div>
                                      </div>
                                    </div>
                                    {/* License pills */}
                                    {activeLicenses.length > 0 && (
                                      <div className="flex flex-wrap gap-1 mt-2 pt-2 border-t border-gray-100">
                                        {activeLicenses.map((lic) => (
                                          <span key={lic} className="inline-block px-1.5 py-0 rounded-full bg-gray-100 text-gray-600 text-[9px]" style={{ fontWeight: 500 }}>
                                            {lic}
                                          </span>
                                        ))}
                                      </div>
                    ...
                    {niosMigrationFlags && (niosMigrationFlags.dhcpOptions.length > 0 || niosMigrationFlags.hostRoutes.length > 0) && (
                      <div className="border-t border-amber-200 bg-amber-50/20">
                        ...
                        >
                          ...
                          <span className="text-[11px] text-[var(--muted-foreground)]">
                            ({niosMigrationFlags.dhcpOptions.length} DHCP option{niosMigrationFlags.dhcpOptions.length !== 1 ? 's' : ''}, {niosMigrationFlags.hostRoutes.length} host route{niosMigrationFlags.hostRoutes.length !== 1 ? 's' : ''})
                          </span>
                        </button>
                        ...
                        {migrationFlagsOpen && (
                          <div className="px-4 pb-4 space-y-4">
                            {/* DHCP Options Table */}
                            {niosMigrationFlags.dhcpOptions.length > 0 && (
                              <div>
                                <h4 className="text-[12px] mb-2 text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>DHCP Options</h4>
                                <div className="overflow-x-auto max-h-[300px] overflow-y-auto rounded border border-[var(--border)]">
                                  <table className="w-full text-[12px]">
                                    ...
                                    <tbody>
                                      {niosMigrationFlags.dhcpOptions.map((opt, idx) => (
                            ...
                            {/* Host Routes Table */}
                            {niosMigrationFlags.hostRoutes.length > 0 && (
                              <div>
                                <h4 className="text-[12px] mb-2 text-[var(--muted-foreground)]" style={{ fontWeight: 600 }}>Host Routes (/32)</h4>
                                <div className="overflow-x-auto max-h-[300px] overflow-y-auto rounded border border-[var(--border)]">
                                  <table className="w-full text-[12px]">
                                    <thead className="sticky top-0 z-10">
                                      <tr className="border-b border-[var(--border)] bg-gray-50">
                                    ...
                                    <tbody>
                                      {niosMigrationFlags.hostRoutes.map((route, idx) => (
                                        <tr key={idx} className="border-b border-[var(--border)] hover:bg-gray-50/50">
