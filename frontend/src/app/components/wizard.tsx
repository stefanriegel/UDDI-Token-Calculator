...
export function Wizard() {
  ...
  const memberSavings = useMemo(() => {
    return resourceSavingsDisplayMembers.map((m) => {
      ...
      const tier = calcServerTokenTier(m.qps, m.lps, serverSizingObjects(m), ff);
      const input = {
        memberId: m.memberId,
  ...
  }, [resourceSavingsDisplayMembers, niosMigrationMap, variantOverrides]);
  ...
  }, [effectiveNiosMetrics, effectiveADMetrics, adMigrationMap, selectedProviders, estimatorServerTokens, serverGrowthBufferPct]);
  ...
  }, [effectiveFindings, effectiveNiosMetrics, effectiveADMetrics, niosMigrationMap, adMigrationMap, selectedProviders, growthBufferPct, serverGrowthBufferPct]);
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
          ...
          {currentStep === 'credentials' && (
            <div>
              <h2 className="text-[18px] mb-1" style={{ fontWeight: 600 }}>
                {isEstimatorOnly ? 'Manual Sizing Estimator' : isNiosOnly && niosMode === 'backup' ? 'Upload NIOS Grid Backup' : 'Choose authentication method'}
              </h2>
              <p className="text-[13px] text-[var(--muted-foreground)] mb-6">
                {isEstimatorOnly
                  ? 'Enter your environment size to estimate Universal DDI token requirements. No credentials or connections needed.'
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              ...
                              <input
                                ...
                                onChange={e => setEstimatorAnswers(prev => ({ ...prev, activeIPs: Math.max(0, parseInt(e.target.value) || 0) }))}
                              />
                            </div>
                            <div>
                              <label className="flex items-center gap-1 text-[12px] text-[var(--muted-foreground)] mb-1" style={{ fontWeight: 500 }}>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                onChange={e => setEstimatorAnswers(prev => ({ ...prev, sites: Math.max(0, parseInt(e.target.value) || 0) }))}
                              />
                            </div>
                            <div>
                              <label className="flex items-center gap-1 text-[12px] text-[var(--muted-foreground)] mb-1" style={{ fontWeight: 500 }}>
                              ...
                              <input
                                ...
                                onChange={e => setEstimatorAnswers(prev => ({ ...prev, networksPerSite: Math.max(0, parseInt(e.target.value) || 0) }))}
                              />
                            </div>
                          </div>
                            ...
                            <div className="flex items-center justify-between mb-2">
                              ...
                              <button
                                ...
                                onClick={() => setEstimatorAnswers(prev => ({
                                  ...prev,
                                  serverEntries: [...prev.serverEntries, { name: `Server ${prev.serverEntries.length + 1}`, formFactor: 'nios-x', qps: 0, lps: 0, objects: 0, tierOverride: '' }],
                                }))}
                                    ...
                                    <div key={idx} className="border border-[var(--border)] rounded-lg p-3 bg-[var(--input-background)]/30">
                                      <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <select
                                          ...
                                          onChange={e => setEstimatorAnswers(prev => ({
                                            ...prev,
                                            serverEntries: prev.serverEntries.map((s, i) => i === idx ? { ...s, formFactor: e.target.value as ServerFormFactor, tierOverride: '' } : s),
                                          }))}
                                        >
                                        ...
                                        <select
                                          ...
                                          onChange={e => {
                                            ...
                                            setEstimatorAnswers(prev => ({
                                              ...prev,
                                              serverEntries: prev.serverEntries.map((s, i) => i === idx
                                                ? {
                                                    ...s,
                                                    tierOverride: tierName,
                                                    ...(selectedTier ? { qps: selectedTier.maxQps, lps: selectedTier.maxLps, objects: selectedTier.maxObjects } : {}),
                                                  }
                                                : s),
                                            }));
                                          }}
                                        >
                                          ...
                                          {(entry.formFactor === 'nios-xaas' ? XAAS_TOKEN_TIERS : SERVER_TOKEN_TIERS).map(t => (
                                      ...
                                      <div className="grid grid-cols-3 gap-3">
                                        <div>
                                          ...
                                          <input
                                            type="number" min={0}
                                            className={`w-full border border-[var(--border)] rounded-lg px-2.5 py-1.5 text-[12px] focus:outline-none focus:ring-1 focus:ring-[var(--infoblox-orange)] ${entry.tierOverride ? 'opacity-50 cursor-not-allowed' : ''}`}
                                            value={entry.qps}
                                            disabled={!!entry.tierOverride}
                                            onChange={e => setEstimatorAnswers(prev => ({
                                              ...prev,
          ...
          {currentStep === 'results' && (
            ...
            <div className="flex-1 min-w-0">
              ...
              {selectedProviders.includes('nios') && (() => {
                ...
                return (
                  <div id="section-migration-planner" className="scroll-mt-6 bg-white rounded-xl border-2 border-[var(--infoblox-blue)]/30 mb-6 overflow-hidden">
                ...
                const totalNiosReplaced = xaasMembers.length; // 1 connection per NIOS member replaced
                    ...
                    <div id="section-member-details" className="scroll-mt-6 border-t border-emerald-200">
                      <button
                      ...
                      {showGridMemberDetails && (
                        <div className="px-4 pb-4">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {displayMembers
                              ...
                              .map((m) => {
                                ...
                                const memberMgmtTokens = effectiveFindings.filter(f => f.provider === 'nios' && f.source === m.memberName).reduce((s, f) => s + f.managementTokens, 0);
                                return (
                                  <div key={m.memberId} className="rounded-lg border border-[var(--border)] bg-white p-3 hover:shadow-sm transition-shadow">
                                    ...
                                    <div className="flex items-center gap-2 mb-2.5 text-[10px] text-[var(--muted-foreground)]">
                                      ...
                                      {m.platform && <PlatformBadge platform={m.platform as any}>{m.platform}</PlatformBadge>}
                                      {niosMigrationMap.has(m.memberName) && (
                                        <span className={`inline-block px-1.5 py-0 rounded text-[9px] text-white ${niosMigrationMap.get(m.memberName) === 'nios-xaas' ? 'bg-purple-500' : 'bg-[var(--infoblox-blue)]'}`} style={{ fontWeight: 600 }}>
                                    ...
                                    <div className="grid grid-cols-4 gap-x-3 gap-y-1.5 text-[11px]">
                                      ...
                                      <div>
                                        ...
                                        <div className="tabular-nums text-[var(--infoblox-orange)]" style={{ fontWeight: 600 }}>{memberMgmtTokens > 0 ? memberMgmtTokens.toLocaleString() : '\u2014'}</div>
                                      </div>
                                      <div>
                                    ...
                                    {(() => {
                                      const savings = memberSavings.find((ms) => ms.memberId === m.memberId);
                          ...
                          {/* Fleet savings totals — RES-06 continuation */}
                          <FleetSavingsTotals fleet={fleetSavings} />
                        </div>
                      )}
                    </div>
