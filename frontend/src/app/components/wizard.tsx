...
export function Wizard() {
  ...
  const hybridScenario = useMemo(() => {
    ...
    return { mgmt: Math.ceil(hybridMgmt * (1 + growthBufferPct)), srv: hybridSrv, selectionCount };
  ...
  const exportCSV = () => {
    ...
    if (selectedProviders.includes('nios') && niosMigrationMap.size > 0) {
      ...
      const allNiosUddi = nf.reduce((s, f) => s + Math.ceil(f.count / (TOKEN_RATES[f.category as keyof typeof TOKEN_RATES] ?? 25)), 0);
                ...
                return (
                  <div id="section-migration-planner" className="bg-white rounded-xl border-2 border-[var(--infoblox-blue)]/30 mb-6 overflow-hidden">
                    ...
                    {effectiveNiosMetrics.length > 0 && (() => {
                      ...
                      // DNS servers, reporting members). Previously this filtered to only
                      // members appearing in findings, which excluded zero-object members.
