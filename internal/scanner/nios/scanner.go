...
func (s *Scanner) Scan(_ context.Context, req scanner.ScanRequest, publish func(scanner.Event)) ([]calculator.FindingRow, error) {
	...
	allZoneRefs := make(map[string]string)    // zone ref → ns_group name (empty if none); tracks ALL zones
	soaPrimaryMap := make(map[string]string)  // zone ref → primary nameserver FQDN (from bind_soa mname)
	...
	// DHCP utilization stats (positional matching like dns/dhcp service).
	var dhcpStaticHosts []string
	var dhcpDynamicHosts []string
	var dhcpTotalHosts []string
	var dhcpUtilization []string

	// License inventory.
	pnodeLicenses := make(map[string][]string)  // physical_oid → license types
	var gridLicenseTypes []string

	// Feature detection (grid-wide).
	gridFeatures := NiosGridFeatures{}

	// physical_oid → vnode ref (for license → member mapping).
	pnodeVnodeMap := make(map[string]string)

	// Deferred physical_node linkage: physical_node objects may appear before
	// virtual_node objects in the XML. Collect them and link after pass 1 completes.
	type physicalNodeInfo struct {
		vnodeRef string
		hwtype   string
		hwplatform string
	}
	var deferredPhysicalNodes []physicalNodeInfo
	...
	if err := streamOnedbXMLFiltered(backupPath, pass1Types, func(props map[string]string) {
		...
		case ".com.infoblox.one.physical_node":
			...
			// XML stream, so vnodeMap may not be populated yet. Collect the data
			// and link after all pass 1 objects are processed.
			vnodeRef := props["virtual_node"]
			if vnodeRef != "" && (hwtype != "" || hwplatform != "") {
				deferredPhysicalNodes = append(deferredPhysicalNodes, physicalNodeInfo{
					vnodeRef:   vnodeRef,
					hwtype:     hwtype,
					hwplatform: hwplatform,
				})
			}
			// Also track physical_oid → virtual_node for license linkage.
			physicalOID := props["physical_oid"]
			if physicalOID != "" && vnodeRef != "" {
				pnodeVnodeMap[physicalOID] = vnodeRef
			}
		...
		case ".com.infoblox.dns.member_dns_properties":
			...
			if props["enable_anycast"] == "true" || props["anycast_enabled"] == "true" {
				gridFeatures.DNSAnycast = true
			}

		case ".com.infoblox.dns.member_dhcp_properties":
			dhcpServiceEnabled = append(dhcpServiceEnabled, props["service_enabled"])
			dhcpStaticHosts = append(dhcpStaticHosts, props["static_hosts"])
			dhcpDynamicHosts = append(dhcpDynamicHosts, props["dynamic_hosts"])
			dhcpTotalHosts = append(dhcpTotalHosts, props["total_hosts"])
			dhcpUtilization = append(dhcpUtilization, props["dhcp_utilization"])
			if props["v6_service_enable"] == "true" {
				gridFeatures.DHCPv6 = true
			}
		...
		case ".com.infoblox.dns.bind_soa":
			...
			// Property name confirmed from Schneider Electric backup (onedb.xml bind_soa objects).
			zoneRef := props["zone"]
			...
			if zoneRef != "" && primaryNS != "" {
				...
				soaPrimaryMap[zoneRef] = primaryNS
			}
		}
	}); err != nil {
	...
	// Link deferred physical_node data to members now that vnodeMap is fully populated.
	// In real NIOS backups (e.g. Schneider), physical_node objects appear before
	// virtual_node objects in the XML, so we must defer this linkage.
	for _, pn := range deferredPhysicalNodes {
		if hostname, ok := vnodeMap[pn.vnodeRef]; ok {
			if mp, ok := memberProps[hostname]; ok {
	...
	for i, hostname := range vnodeOrder {
		...
		if i < len(dhcpStaticHosts) {
			props["_static_hosts"] = dhcpStaticHosts[i]
		}
		if i < len(dhcpDynamicHosts) {
			props["_dynamic_hosts"] = dhcpDynamicHosts[i]
		}
		if i < len(dhcpTotalHosts) {
			props["_total_hosts"] = dhcpTotalHosts[i]
		}
		if i < len(dhcpUtilization) {
			props["_dhcp_utilization"] = dhcpUtilization[i]
		}
	}
	...
	hostnameLookup := make(map[string]string, len(vnodeMap))
	for _, hostname := range vnodeMap {
		hostnameLookup[strings.ToLower(hostname)] = hostname
	}

	// Resolve zones using three-tier fallback: ns_group → SOA mname → Grid Master.
	zoneMemberMap := make(map[string]string, len(allZoneRefs))

	// First pass: resolve via ns_group (existing logic).
	...
	// Second pass: SOA fallback for unresolved zones (including zones without ns_group).
	for zoneRef := range allZoneRefs {
		if _, resolved := zoneMemberMap[zoneRef]; resolved {
			continue
		}
		if soaMname, ok := soaPrimaryMap[zoneRef]; ok {
		...
		// Grid Master final fallback (D-01).
		zoneMemberMap[zoneRef] = gmHostname
	}
	...
	// Build per-member license map from product_license → physical_node → virtual_node chain.
	memberLicenses := make(map[string]map[string]bool)
	for physOID, licTypes := range pnodeLicenses {
		vnodeRef := pnodeVnodeMap[physOID]
		if vnodeRef == "" {
			continue
		}
		hostname, ok := vnodeMap[vnodeRef]
		if !ok {
			continue
		}
	...
	if len(result.dhcpOptions) > 0 || len(result.hostRoutes) > 0 {
		...
		s.migrationFlags = &NiosMigrationFlags{
			DHCPOptions: result.dhcpOptions,
...
// Real NIOS backup values include short codes like "VMW" and "HW" (from Schneider backup),
