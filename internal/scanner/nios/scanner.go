...
func (s *Scanner) Scan(_ context.Context, req scanner.ScanRequest, publish func(scanner.Event)) ([]calculator.FindingRow, error) {
	...
	if err := streamOnedbXMLFiltered(backupPath, pass1Types, func(props map[string]string) {
		...
		case ".com.infoblox.one.physical_node":
			...
			// Link physical_node to its virtual_node via the "virtual_node" property,
			// which contains the virtual_oid value of the parent virtual_node.
			// Confirmed from Schneider backup: physical_node has virtual_node="10"
			// matching virtual_node's virtual_oid="10".
			vnodeRef := props["virtual_node"]
			if vnodeRef != "" {
				if hostname, ok := vnodeMap[vnodeRef]; ok {
					if mp, ok := memberProps[hostname]; ok {
						mp["_hwtype"] = hwtype
						mp["_hwplatform"] = hwplatform
					}
				}
			}
