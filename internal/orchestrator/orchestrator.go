			...
			if err != nil {
				...
				if nrs, ok := s.(scanner.NiosResultScanner); ok {
					if encoded := nrs.GetNiosServerMetricsJSON(); len(encoded) > 0 {
						...
						// Merge QPS data from Splunk XML upload (if available)
						// into the server metrics before they are used for tier calcs.
						if qpsJSON := sess.NiosQPSDataJSON; len(qpsJSON) > 0 {
							var metrics []nios.NiosServerMetric
							var qpsData map[string]float64
							if json.Unmarshal(encoded, &metrics) == nil && json.Unmarshal(qpsJSON, &qpsData) == nil {
								merged := nios.MergeQPSData(metrics, qpsData)
								if mergedJSON, err := json.Marshal(merged); err == nil {
									sess.SetNiosServerMetricsJSON(mergedJSON)
								}
							}
						}
					}
